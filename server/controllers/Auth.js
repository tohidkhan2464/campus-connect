...
//         const otpdetails = await OTP.find().sort({createdAt:-1}).limit(1);
//         const recentOTP = otpdetails.otp;
//         return res.json({
//             success:true,
//             message:"Otp fetch",
//             recentOTP:recentOTP,
...
exports.signUp = async (req, res) => {
    try {
        ...
        const existingUserName = await Profile.findOne({userName:userName});
        if(existingUserName){
            return res.status(400).json({
                success: false,
        ...
        const profileDetails = await Profile.create({userName:userName });
...
exports.login = async (req, res) => {
    try {
        ...
        const userDetails = await User.find().sort({createdAt:-1}).limit(1);
